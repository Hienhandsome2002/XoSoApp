...
const MienTrung = ({ navigation }) => {
    ...
    const getAPI = () => {
        const url = `https://api.xoso.me/app/json-kq-mientrung?name=KQXS&v=2&ngay_quay=${moment(date).format('YYYY-MM-DD')}`
        fetch(url)
            .then((res) => res.json())
            .then((resJson) => {
                var a = []
    ...
    return (
        <ScrollView>
            <View>
                <Header title='Xổ Số Miền Trung' type='arrow-left' navigation={navigation}></Header>
                        ...
                        data.length > 0 && data.map(item => (
                            <View>
                                ...
                                {item.dataProcess.map(i => {
                                    ...
                                    return (
                                        <View style={{ flexDirection: 'row', justifyContent: 'space-around', alignItems: 'center', marginHorizontal: 12, borderBottomWidth: 1, borderColor: '#EEEEEE', backgroundColor: i.rank % 2 != 0 ? '#F8F8FF' : 'white' }}>
